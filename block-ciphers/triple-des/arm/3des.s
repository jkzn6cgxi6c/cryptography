.globl triple_des_key_schedule @ void triple_des_key_schedule(unsigned char key[288])
.globl triple_des_encrypt      @ void triple_des_encrypt(unsigned char buffer[8], const unsigned char key[288])
.globl triple_des_decrypt      @ void triple_des_decrypt(unsigned char buffer[8], const unsigned char key[288])
...
triple_des_key_schedule:
    stmdb   sp!, {r4-r6, r11}           @ save registers
    add     r12, r0, #8                 @ pointer to 2nd key
    ldmia   r12, {r1-r4}                @ load 2nd & 3rd key
    add     r12, #88                    @ pointer to 2nd expanded key
    stmia   r12!, {r1-r2}               @ store 2nd key
    add     r12, #88                    @ pointer to 3rd expanded key
    stmia   r12!, {r3-r4}               @ store 3rd key
    add     r12, #88                    @ pointer to end of keys
    mov     r4, #0                      @ temporary variable to hold Kn
    sub     r11, pc, #940               @ pointer to PC-2
.Ltriple_des_key_schedule_round:
...
.Ldes_key_schedule_round_pc_2_a:
    and     r3, r1, #15
    ldr     r3, [r11, r3, LSL #2]       @ next group of bits of Kn
    mov     r1, r1, ROR #4
    orr     r4, r3
    add     r11, #64
    subs    r5, #1
...
.Ldes_key_schedule_round_pc_2_b:
    ...
    mov     r4, r4, LSR #8              @ r4 = 0
    sub     r11, #896                   @ pointer to PC-2
    teq     r6, #0
    bne     .Ldes_key_schedule_round
    teq     r0, r12
    bne     .Ltriple_des_key_schedule_round
    ldmia   sp!, {r4-r6, r11}           @ restore saved registers
    mov     pc, lr

triple_des_encrypt:
    stmdb   sp!, {r4-r9, r11, lr}
    ldmia   r0, {r2-r3}                 @ L, R
    bl      .Ldes_ip
    mov     r4, #0
    bl      .Ldes_encrypt
...
.Ldes_ip:
    sub     r11, pc, #3472              @ pointer to sbox1
    eor     r4, r2, r2, ROR #16
    bic     r4, #16711680
    mov     r2, r2, ROR #8
    eor     r2, r4, LSR #8
